...
switch ($format) {
    ...
    case 'lgpd':
        $output = CnpjRule::maskLgpd($value);
        break;

    case 'formatted':
    default:
        $output = CnpjRule::format($value);
        break;
}

echo '<span class="field-cnpj" data-field-id="' . (int) $field->id . '">' . htmlspecialchars($output, ENT_QUOTES, 'UTF-8') . '</span>';
