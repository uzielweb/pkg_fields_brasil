...
namespace Uziel\Plugin\Fields\Telefone\Extension;
...
/**
 * Fields Telefone / Celular / WhatsApp Plugin
 *
 * @since 1.0.0
 */
final class Telefone extends FieldsPlugin implements SubscriberInterface
{
    /**
    ...
    public function onCustomFieldsPrepareDom($field, \DOMElement $parent, Form $form)
    {
        ...
        FormHelper::addRulePath(JPATH_PLUGINS . '/fields/telefone/rules');
        FormHelper::addRulePrefix('Uziel\\Plugin\\Fields\\Telefone\\Rule');

        // Configure validation and UX attributes
        $fieldNode->setAttribute('validate', 'telefone');

        $currentClass = $fieldNode->getAttribute('class');
        $fieldNode->setAttribute('class', trim($currentClass . ' validate-telefone joomla-field-telefone'));

        $applyMask = $field->fieldparams ? $field->fieldparams->get('apply_mask', '1') : '1';
        $fieldNode->setAttribute('data-apply-mask', (string) $applyMask);
        $fieldNode->setAttribute('maxlength', '15');

        if (!$fieldNode->getAttribute('hint')) {
            $fieldNode->setAttribute('hint', '(00) 00000-0000');
        }
